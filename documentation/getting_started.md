...
```bash
quizgen generate --allow_generator --yaml example_files/example_exam.yaml --num_pdfs 1
```

PDFs are generated in `out/`.
`example_files/example_exam.yaml` includes a `FromGenerator` question, so `--allow_generator` is required.
...
```bash
quizgen generate --allow_generator --yaml example_files/example_exam.yaml --num_pdfs 1 --optimize_space
```
...
```bash
quizgen generate --allow_generator --yaml example_files/example_exam.yaml --num_canvas 5 --course_id 12345
```
