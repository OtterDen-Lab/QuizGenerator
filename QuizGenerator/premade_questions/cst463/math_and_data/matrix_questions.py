        ...
        if subpart_index == 0 and not self.is_multipart():
            ...
            for i in range(rows):
                for j in range(cols):
                    ...
                    self.answers[answer_key] = ContentAST.Answer.integer(answer_key, result[i][j])
        else:
    ...
    def create_subquestion_answers(self, subpart_index, result):
        ...
        if subpart_index == 0 and not self.is_multipart():
            ...
            if result is not None:
                self.answers["result_rows"] = ContentAST.Answer.integer("result_rows", self.result_rows,
                                                             label="Number of rows in result")
                self.answers["result_cols"] = ContentAST.Answer.integer("result_cols", self.result_cols,
