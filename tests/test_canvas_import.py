...
def test_export_canvas_quiz_flatten_groups_and_download_images(tmp_path):
    ...
    picture_nodes = exported["questions"][0]["kwargs"]["yaml_spec"]["body"]
    assert any("picture" in node for node in picture_nodes)

    image_path = Path(picture_nodes[1]["picture"]["path"])
